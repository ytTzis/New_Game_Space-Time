Shader "MK4/Rain Alpha"
{
    Properties
    {
        ...
        _Color0("Color 0", Color) = (0.5807742,0.7100198,0.9632353,1)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                half4 albedo = MK4BuildLitColor(input.uv, input.positionWS, 1.0.xxxx);
                clip(albedo.a - _Cutoff);
                albedo.rgb = lerp(albedo.rgb, albedo.rgb * _Color0.rgb, saturate(_RainMask * 0.2));
                ...
                half3 lit = MK4ApplySimpleLighting(albedo.rgb * lerp(1.0h, ao, 0.5h), max(specGloss, _Specular.xxx), _Smoothness, normalWS, viewDirWS, input.positionWS, input.shadowCoord);
